document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function appliquerFiltreCombiné() {
    ...
    let filtrées = autruches.filter(a =>
      a.age > 3 &&
      a.poids > 100 &&
      (selectedRanch === "" || a.ranch === selectedRanch)
    );

    // ⚠ Si ranch = "Les Pondeuses", on garde seulement les femelles
    if (selectedRanch === "Les Pondeuses") {
      filtrées = filtrées.filter(a => a.genre.toLowerCase() === "femelle");
    }
  ...
  ranchSelect.addEventListener("change", () => {
    const selectedRanch = ranchSelect.value;
    let filtrées = selectedRanch
      ? toutesLesAutruches.filter(a => a.ranch === selectedRanch)
      : toutesLesAutruches;

    // ⚠ Si ranch = "Les Pondeuses", uniquement femelles
    if (selectedRanch === "Les Pondeuses") {
      filtrées = filtrées.filter(a => a.genre.toLowerCase() === "femelle");
    }
