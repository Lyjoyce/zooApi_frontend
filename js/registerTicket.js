...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // URL backend configurable (évite toute redirection en prévol)
  const isLocalDev = ["localhost", "127.0.0.1", "0.0.0.0"].includes(window.location.hostname);
  // Mets ici TON vrai domaine API (qui ne redirige pas). Idéalement un sous-domaine direct type api.ton-domaine.com.
  const API_BASE_URL = isLocalDev
    ? "http://localhost:8080/api/v1/tickets"
    : "https://api.ton-domaine.com/api/v1/tickets";
  ...
  form.addEventListener("submit", async (e) => {
    ...
    const payload = {
      ...
      nbEnfants: parseInt(nbEnfants.value, 10),
      nbAdultes: parseInt(nbAdultes.value, 10),
      ateliers: ateliers
    };
